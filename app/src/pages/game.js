...
export const Game2049Page = () => {
    ...
    const [boardDisplay, setBoardDisply] = useState([]);
    const [initialized, setInitialized] = useState(false);
    /*
    useEffect(() => {
        if (!initialized) { $(document).ready(gameStart); setInitialized(true); }

    }, [initialized, gameStart]);
    */
    useEffect(() => {
        console.log("score: ", window.game.score)
        if (window?.game?.score? true: false) {
            setScore(window.game.score)
        }
    }, [window.game.score])
    ...
    class Game extends React.Component {
        constructor(size) {
            ...
            Game.prototype.initTile = function () {
                ...
                var tile = new Tile(emptyCell.x, emptyCell.y, window.game);
                // isGameOver determines whether the game is finished; needs to be run: before and after creating tile
                this.isGameOver();
                //
            };
            ...
            Game.prototype.move = function (getDirection) {
                ...
                gameBoard.forEach(function (val, index, array) {
                    if (val.tilesArray.length) {
                      val.tilesArray.forEach(function (tile) {
        ...
        <div>
            <main class="wrapper">
                ...
                <div class="intro clearfix">
                    <h1 class="intro_title"> 2049 </h1>
                </div>

                <div class="controls clearfix">
                    <div class="controls_game">
                        <button data-js="newGame" class="cta-button" onClick={
                            gameStart
                            }> New Game </button>
                            <button data-js="newGame" class="cta-button" onClick={
                            buyTicket
